...
class radfil(object):
    ...
    def make_fil_spine(self,beamwidth = None,verbose = False):
        ...
        fils.analyze_skeletons(verbose=verbose,skel_thresh=170*u.pix,branch_thresh =170 * u.pix, prune_criteria='length')
    ...
    def build_profile(self, pts_mask = None, samp_int=3, bins = None, shift = True, fold = False, make_cuts = True, cutdist=None):
        ...
        # -----------------Added argument-----------
        ## filangle
        if isinstance(filangle, float):
            self.filangle = float(filangle)
        else:
            self.filangle = None
            raise TypeError("filangle has to be an float. See documentation.")
